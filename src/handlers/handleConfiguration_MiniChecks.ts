...
export interface handleConfiguration_MiniChecks {
}

export async function handleExpensiveStatements(params: handleConfiguration_MiniChecks): Promise<ToolResponse> {
  try {
    const query = `
WITH 
...
( SELECT
  ...
  ( SELECT TOP 1
      IFNULL(OS_NAME, 'n/a') OS_NAME,
      IFNULL(OS_PPMS_NAME, 'n/a') OS_PPMS_NAME
    FROM
      DUMMY LEFT OUTER JOIN
    ( SELECT
        HOST,
...
SELECT
  CASE 
    ...
    WHEN ONLY_POTENTIALLY_CRITICAL_RESULTS = 'X' OR ROW_NUM = 1 OR ORDER_BY = 'HOST' THEN LPAD(MAP(CHECK_ID_PREFIX, '', TO_VARCHAR(CHECK_ID), CHECK_ID_PREFIX || LPAD(CHECK_ID, 4, 0)), 5) 
    ELSE '' 
  END CHID,
  CASE WHEN ONLY_POTENTIALLY_CRITICAL_RESULTS = 'X' OR ROW_NUM = 1 OR ORDER_BY = 'HOST' THEN DESCRIPTION ELSE '' END DESCRIPTION,
  IFNULL(HOST, '') HOST,
  MAP(VALUE, '999999', 'n/a', '999999.00', 'n/a', '-999999', 'n/a', '-999999.00', 'n/a', NULL, 'n/a', 
...
/* TMC_GENERATION_START_1 */
  ( SELECT
    ...
    ( SELECT
        'LICENSE_LIMIT',
        '',
        CASE WHEN PRODUCT_LIMIT = 0 THEN '0' ELSE TO_VARCHAR(TO_DECIMAL(ROUND(LEAST(TOTAL_ALLOC_LIM_GB, PRODUCT_USAGE) / PRODUCT_LIMIT * 100), 10, 0)) END
      FROM
        M_LICENSE,
        GENERAL_INFO
    )
    ...
    ( SELECT
      ...
      ( SELECT
          ...
          ( SELECT SUM(IFNULL(BF.COMPRESSED_SIZE, BF.BACKUP_SIZE)) / 1024 / 1024 / 1024 FROM TEMP_M_BACKUP_CATALOG_FILES BF WHERE BF.BACKUP_ID = B.BACKUP_ID ) BACKUP_SIZE_GB
        FROM
    ...
    ( SELECT
        ...
        '', 
        IFNULL(TO_VARCHAR(GREATEST(0, SECONDS_BETWEEN(MIN(UPDATE_TRANSACTION_START_TIME), CURRENT_TIMESTAMP))), '0')
      FROM
        TEMP_M_TRANSACTIONS
      WHERE
      ...
      ( SELECT
          ...
          MAP(TABLE_NAME, NULL, 'none', TABLE_NAME || MAP(PART_ID, 0, '', CHAR(32) || '(' || PART_ID || ')') || CHAR(32) || '(' || 
            MAP(C.NAME, 'LARGE_MEMORY_TABLES', TO_DECIMAL(ROUND(T.MEMORY_SIZE_IN_TOTAL / 1024 / 1024 / 1024), 10, 0) || CHAR(32) || 'GB)', 
              'LARGE_ALLOC_LIM_TABLES', TO_DECIMAL(ROUND(T.MEMORY_SIZE_IN_TOTAL / T.GLOBAL_ALLOCATION_LIMIT * 100), 10, 0) || CHAR(32) || '%)')) VALUE,
          ROW_NUMBER() OVER (PARTITION BY C.NAME ORDER BY T.MEMORY_SIZE_IN_TOTAL DESC) ROWNUM
        FROM
        ...
        ( SELECT
          ...
          WHERE
            T.HOST = H.HOST AND
            ( (T.MEMORY_SIZE_IN_TOTAL + T.PERSISTENT_MEMORY_SIZE_IN_TOTAL) / 1024 / 1024 / 1024 > CASE WHEN TABLE_NAME = 'ACDOCA' THEN 100 ELSE 50 END OR
    ...
    UNION ALL
    ( SELECT
        MAP(ID, 5028, 'LONG_RUNNING_INTERVAL', 5034, 'THREAD_SAMPLES_INTERVAL', 'THREAD_CALLSTACKS_INTERVAL'),
        '',
        TO_VARCHAR(INTERVALLENGTH)
      FROM
        _SYS_STATISTICS.STATISTICS_SCHEDULE
      WHERE
        ID IN ( 5028, 5034, 5057 )
    )
    UNION ALL
    ( SELECT
        C.NAME,
    ...
    ( SELECT
      ...
      ( SELECT 'QCM_TABLES' NAME, 'QCM%' PATTERN FROM DUMMY UNION ALL
        ...
        SELECT 'ZBICZ_TABLES',    'ZBICZ%'       FROM DUMMY UNION ALL
        SELECT '1LT_DS_TABLES',   '/1LT/DS%'     FROM DUMMY
      ) N LEFT OUTER JOIN
        TEMP_TABLES T ON
      ...
      ( SELECT
        ...
        ( SELECT 
          ...
          FROM
            M_HEAP_MEMORY
          WHERE
            CATEGORY LIKE 'Pool/LVCAllocator/LVCContainerDir%' OR
            CATEGORY = 'Pool/LVCAllocator/LVCObjectPageDir'
          GROUP BY
            HOST
        )
        GROUP BY
    ...
    ( SELECT
      ...
      FROM
        STATEMENT_HINTS
      WHERE
        COMMENTS NOT LIKE 'SAP Note 2700051%'
    )
    UNION ALL
    ( SELECT
        'UDIV_OVERHEAD',
    ...
    ( SELECT
        ...
        '',
        CASE
          WHEN OS_PPMS_NAME = 'n/a'                                                                                                                        THEN 'n/a'
    ...
    ( SELECT /* Needs to be able to extract the first up to three numbers after the first "-" (e.g. 0, 47, 71) from versions like 
        ...
        CASE 
          ...
          WHEN OS_NAME LIKE 'SUSE Linux Enterprise Server%12%5'                    AND ( KV_1 = 122  AND KV_2 = 212 )                                                      THEN 'no' || CHAR(32) || '(' || KV || ')' /* SAP Note 3525559 */
      ...
      ( SELECT
        ...
        ( SELECT
          ...
          ( SELECT
              ...
              MAX(MAP(KEY, 'os_name', VALUE)) OS_NAME,
              MAX(MAP(KEY, 'os_ppms_name', VALUE)) OS_PPMS_NAME,
              MAX(MAP(KEY, 'os_kernel_version', REPLACE(VALUE, 'kernel-default-', ''))) KV
            FROM
              M_HOST_INFORMATION
            GROUP BY
              HOST
    ...
    ( SELECT
        ...
        HOST,
        MAP(MAX(VIRTUAL_ADDRESS_SPACE_TOTAL_SIZE), 0, 'n/a', TO_VARCHAR(TO_DECIMAL(MAX(VIRTUAL_ADDRESS_SPACE_USED_SIZE) / MAX(VIRTUAL_ADDRESS_SPACE_TOTAL_SIZE) * 100, 10, 0)))
      FROM
        M_SERVICE_MEMORY
      GROUP BY
        HOST
        ...
        ( SELECT
          ...
          WHERE
            ...
            T.LOCK_WAIT_NAME NOT IN ('', CHAR(63), 'capacityReached', 'ChannelUtilsSynchronousCopyHandler', 'CSPlanExecutorLock', 'CSPlanExecutorWaitForResult', 
              ...
              'StartSecondExclusiveMergePhase', 'TableLockWaitCondStat', 'TransactionLockWaitCondStat', 'TransactionLockWaitSmpStat') AND
    ...
    ( SELECT
        ...
        CASE
          ...
          WHEN C.NAME = 'AVG_IO_READ_LATENCY'      THEN TO_VARCHAR(CASE WHEN SUM(I.TOTAL_READ_TIME) < 200000000 OR SUM(I.TOTAL_READ_SIZE) / GREATEST(1, SUM(I.TOTAL_READS)) > 10000000 THEN 999999 ELSE TO_DECIMAL(SUM(I.TOTAL_READ_TIME) / SUM(I.TOTAL_READS) / 1000, 10, 2) END)
    ...
    ( SELECT
      ...
      ( SELECT
          L.HOST,
          SUM(GREATEST(L.DATA_READ_SIZE, 0)) / 1024 / 1024 READ_SIZE_MB,
          SUM(GREATEST(L.DATA_READ_TIME, 0)) / 1000000 READ_TIME_S
        FROM
          DUMMY LEFT OUTER JOIN
        ( SELECT
            HOST,
            MAX(TIMESTAMP) START_TIME
          FROM
            M_MERGED_TRACES
          WHERE
            SERVICE_NAME = 'indexserver' AND
            TRACE_TEXT LIKE 'Starting reloading column%'
          GROUP BY
            HOST
        ) S ON
          1 = 1 LEFT OUTER JOIN
        ( SELECT
          ...
          WHERE
            SERVICE_NAME = 'indexserver' AND
            TRACE_TEXT LIKE 'Finished table reloading%'
          GROUP BY
            HOST
         ) E ON
           1 = 1 LEFT OUTER JOIN
          _SYS_STATISTICS.HOST_LOAD_HISTORY_SERVICE L ON
            S.HOST = E.HOST AND
            E.HOST = L.HOST AND
            E.END_TIME > S.START_TIME AND
            L.TIME BETWEEN S.START_TIME AND E.END_TIME
        GROUP BY
          L.HOST
      )
    )
    ...
    ( SELECT
      ...
      ( SELECT
        ...
        WHERE
          C.BACKUP_ID = CF.BACKUP_ID AND
          SECONDS_BETWEEN(C.SYS_END_TIME, CURRENT_TIMESTAMP) <= 86400 * BI.MIDTERM_DAYS
      )
    )
    UNION ALL
    ( SELECT
    ...
    ( SELECT
      ...
      WHERE
       STATUS = 'Inactive' AND
       ID NOT IN (1, 20, 41, 58, 73, 74, 77, 95, 96, 117, 127, 5008, 5024, 5025, 5033, 5035, 5059)
    )
    UNION ALL
    ( SELECT
...
/* Available as of SAP HANA 2.00.030 */
    ...
    ( SELECT
      ...
      ( SELECT 'async_io_limit' KEY_FIGURE    FROM DUMMY UNION ALL
        ...
        SELECT 'net_port_unreserved_ranges'   FROM DUMMY UNION ALL
    ...
    ( SELECT
        ...
        '',
        CASE
          WHEN H.KEY IS NULL                                                  THEN 'n/a'
          WHEN ( I.OS_PPMS_NAME LIKE 'LINUX_PPC64%' OR
                 I.OS_NAME BETWEEN 'RHEL 5.0' AND 'RHEL 8.99' OR
                 I.OS_NAME LIKE 'RHEL 9.%' AND I.OS_NAME <= 'RHEL 9.1' OR
                 I.OS_NAME BETWEEN 'SLES 10' AND 'SLES 14.99' OR
                 I.OS_NAME LIKE 'SLES 15.%' AND I.OS_NAME <= 'SLES 15.4'
               ) AND
               H.VALUE != 'always madvise [never]'                            THEN 'no'
          WHEN ( I.OS_PPMS_NAME NOT LIKE 'LINUX_PPC64%' AND
                 ( I.OS_NAME LIKE 'RHEL 9.%' AND I.OS_NAME >= 'RHEL 9.2' OR
                   I.OS_NAME BETWEEN 'RHEL 10' AND 'RHEL 40' OR
                   I.OS_NAME LIKE 'SLES 15.%' AND I.OS_NAME >= 'SLES 15.5' OR
                   I.OS_NAME BETWEEN 'SLES 16' AND 'SLES 40'
                 ) 
               ) AND 
               H.VALUE != 'always [madvise] never'                            THEN 'no'
          ELSE 'yes'
        END
      FROM
        GENERAL_INFO I LEFT OUTER JOIN
        ( SELECT TOP 1 KEY, VALUE FROM M_HOST_INFORMATION WHERE KEY = 'kernel.transparent_hugepage.enabled' ) H ON
          1 = 1
    )
    UNION ALL
    ( SELECT
    ...
    ( SELECT
        ...
        '',
        CASE WHEN MAX(D.DATA_TYPE) = 'BIGINT' AND MIN(S.MAX_VALUE) >= 4000000000000000000 THEN 'n/a' ELSE TO_VARCHAR(TO_DECIMAL(GREATEST(0, MAX(MS.CURRENT_VALUE) / 2147483648 * 100), 10, 2)) END                          
      FROM
    ...
    ( SELECT
      ...
      ( SELECT TOP 1 HOST, MAX_SIZE / 1024 / 1024 / 1024 BUFFER_GB FROM M_BUFFER_CACHE_STATISTICS WHERE MAX_SIZE > 0 ORDER BY ALLOCATED_SIZE DESC ) B
      WHERE
        B.HOST = W.HOST
    )
    UNION ALL
        ...
        ( SELECT DISTINCT
          ...
          ( SELECT
            ...
            FROM
              BASIS_INFO BI,
              ( SELECT
                  SUM(SUM(TOTAL_EXECUTION_TIME)) OVER (PARTITION BY STATEMENT_HASH) SUM_TOTAL_TIME_MS,
                  SERVER_TIMESTAMP,
                  STATEMENT_HASH,
                  EXECUTION_ENGINE,
            ...
            HAVING
              ...
              AVG(SUM_TOTAL_TIME_MS) >= 1800 * 1000 * 1000
          )
        )
        WHERE
    ...
    ( SELECT
        ...
        '',
        IFNULL(USER_NAME || CHAR(32) || '(' || 
          MAP(VALUE, '0', 'unlimited', VALUE || CHAR(32) || 'GB') || ')', 'none')
      FROM
        DUMMY LEFT OUTER JOIN
        USER_PARAMETERS ON
    ...
    ( SELECT
      ...
      FROM
      ( SELECT MAX(VALUE) SOCKETS FROM M_HOST_INFORMATION WHERE KEY = 'cpu_threads' ) LEFT OUTER JOIN
        USER_PARAMETERS ON
    ...
    UNION ALL
    ( SELECT TOP 10
        'ORPHAN_SLT_LOGGING_TABLES',
        '',
        IFNULL(TABLE_NAME || CHAR(32) || '(' || RECORD_COUNT || CHAR(32) || 'record' || MAP(RECORD_COUNT, 1, '', 's') || ')', 'none')
      FROM
        DUMMY LEFT OUTER JOIN
      ( SELECT
          TABLE_NAME,
        ...
        FROM
          TEMP_M_TABLES TA
        WHERE
          ( TA.TABLE_NAME LIKE '/1CADMC/%' OR TA.TABLE_NAME LIKE '/1DH/ML%' ) AND
          NOT EXISTS ( SELECT 1 FROM TRIGGERS TR WHERE TR.DEFINITION LIKE '%' || TA.TABLE_NAME || '%' )
      ) ON
        1 = 1
      ORDER BY
        RECORD_COUNT DESC,
    ...
    ( SELECT
        'LAST_AUTO_MERGE',
        '',
        IFNULL(TO_VARCHAR(TO_DECIMAL(SECONDS_BETWEEN(MAX(START_TIME), CURRENT_TIMESTAMP) / 3600, 10, 0)), 'never')
      FROM
        DUMMY LEFT OUTER JOIN
        M_DELTA_MERGE_STATISTICS ON
          TYPE = 'MERGE' AND
          MOTIVATION = 'AUTO'
    )
    UNION ALL
    ( SELECT
        'RAW_RECORD_OVERHEAD',
        '',
        MAP(TABLE_NAME, NULL, 'none', TABLE_NAME || CHAR(32) || MAP(PART_ID, 0, '', '''' || PART_ID || '''') || '(' || TO_DECIMAL(RECORD_COUNT / 1000000, 10, 0) || CHAR(32) || 'Mio. rows vs.' || CHAR(32) || 
          TO_DECIMAL(( RAW_RECORD_COUNT_IN_MAIN + RAW_RECORD_COUNT_IN_DELTA ) / 1000000, 10, 0) || CHAR(32) || 'Mio. raw rows)')
      FROM
        DUMMY LEFT OUTER JOIN
        TEMP_M_CS_TABLES ON
         RECORD_COUNT > 1000000 AND
         ( RAW_RECORD_COUNT_IN_MAIN + RAW_RECORD_COUNT_IN_DELTA ) > 10 * RECORD_COUNT
    )
...
/* TMC_GENERATION_START_2 */
    ...
    ( SELECT  213, 'EMPTY_HOST_AGENT_METRICS',     'Missing host agent metrics',                      '2100040', '=',        'no',            -1,    -1 FROM DUMMY ) UNION ALL
    ...
    ( SELECT  269, 'OS_THP_ACTIVE',                'Proper transparent huge page configuration',      '2131662', '=',        'yes',           -1,    -1 FROM DUMMY ) UNION ALL
    ( SELECT  270, 'UNKNOWN_HARDWARE',             'Unknown hardware components',                     '1828631', '=',        '0',             -1,    -1 FROM DUMMY ) UNION ALL
    ( SELECT  271, 'OS_NET_PORT_UNRESERVED_RANGES','Unreserved SAP HANA ports',                       '401162',  '=',        '-',             -1,    -1 FROM DUMMY ) UNION ALL
    ...
    ( SELECT  452, 'UNIQUE_INDEXES_PCT',           'Size of unique indexes (%)',                      '2160391', '<=',       '5.00',          -1,    -1 FROM DUMMY ) UNION ALL
    ...
    ( SELECT  557, 'LAST_AUTO_MERGE',              'Time since last auto merge (h)',                  '2057046', '<=',       '12',            -1,    -1 FROM DUMMY ) UNION ALL
    ...
    ( SELECT  578, 'RAW_RECORD_OVERHEAD',          'Tables with high raw record count',               '',        '=',        'none',          -1,    -1 FROM DUMMY ) UNION ALL
    ...
    ( SELECT  641, 'NUM_TRACEFILES_SHORTTERM',     'Number of trace files / day (short-term)',        '2119087', '<=',       '35',            -1,    -1 FROM DUMMY ) UNION ALL
    ...
    ( SELECT  754, 'LONG_RUNNING_INTERVAL',        'Historic long running statements interval (s)',   '2147247', '=',        '300',           -1,    -1 FROM DUMMY ) UNION ALL
    ...
    ( SELECT  942, 'CATALOG_BACKUP_SIZE_SHARE',    'Catalog size share (%, mid-term)',                '2505218', '<=',       '5.00',          -1,    -1 FROM DUMMY ) UNION ALL
    ...
    ( SELECT 1852, 'SR_LOGREPLAY_BACKLOG_CURR',    'Current log replay backlog (GB)',                 '1999880', '<=',       '10',            -1,    -1 FROM DUMMY ) UNION ALL
    ( SELECT 1853, 'SR_LOGREPLAY_DELAY_CURR',      'Current log replay delay (s)',                    '1999880', '<=',       '600',           -1,    -1 FROM DUMMY ) UNION ALL
    ( SELECT 1854, 'SR_LOGREPLAY_BACKLOG_HIST',    'Max. log replay backlog (GB, long-term)',         '1999880', '<=',       '30',            -1,    -1 FROM DUMMY ) UNION ALL
    ( SELECT 1855, 'SR_LOGREPLAY_DELAY_HIST',      'Max. log replay delay (s, long-term)',            '1999880', '<=',       '1800',          -1,    -1 FROM DUMMY ) UNION ALL
    ...
    ( SELECT 2511, 'ORPHAN_SLT_LOGGING_TABLES',    'Orphan SLT logging tables',                       '2014562', '=',        'none',          -1,    -1 FROM DUMMY ) UNION ALL
    ( SELECT 2515, '1LT_DS_TABLES',                'Migration cockpit staging tables',                '2733253', '<=',       '1000',          -1,    -1 FROM DUMMY ) UNION ALL
...
/* TMC_GENERATION_END_2 */
  ...
  WHERE
    ...
    ( GI.DB_TYPE = 'TENANT' OR 
      ...
      CC.CHECK_ID NOT IN (314, 364, 371, 385, 411, 471, 474, 551, 552, 718, 744, 750, 751, 753, 754, 915, 916, 942, 1031, 1128, 1142, 1165, 1168, 1170, 1550, 2113, 2115, 2118)
    ) AND
    ( GI.DB_TYPE = 'TENANT' OR
      BI.SKIP_OVERLAPPING_CHECKS_IN_SYSTEMDB = ' ' OR
      CC.CHECK_ID NOT IN (12, 208, 209, 213, 221, 222, 236, 245, 250, 269, 409, 410, 413, 811, 862, 866, 1031, 1159, 1160, 1333, 1410, 1846)
    )
) M
WHERE
