...
async function handleExpensiveStatements(params) {
    try {
        ...
        const { beginTime = '1000/10/18 07:58:00', endTime = '9999/10/18 08:05:00', timezone = 'SERVER', host = '%', port = '%', connectionId = -1, statementHash = '%', workloadClass = '%', appUser = '%', dbUser = '%', sqlPattern = '%', appSource = '%', bindValues = '%', sessionVariables = '%', errorCode = -1, errorText = '%', onlyErrors = false, sqlTextOutputLength = 40, minMemoryGB = -1, maxMemoryGB = -1, minDurationS = -1, minSqlTextLength = -1, operations = '%', aggregateBy = 'NONE', timeAggregateBy = 'NONE', orderBy = 'TIME', resultRows = -1 } = params;
        const query = `
SELECT
/* 
[NAME]
...
ORDER BY
  ROW_NUM
    `;
        ...
        return (0, utils_1.formatQueryResult)(result, '💸 HANA 오래걸리거나 메모리 사용율이 높은 구문 분석');
    }
    catch (error) {
        console.error('Expensive statements query failed:', error);
        return (0, utils_1.createErrorResponse)(`오래걸리거나 메모리 사용율이 높은 구문 조회 실패: ${error instanceof Error ? error.message : String(error)}`);
    }
}
