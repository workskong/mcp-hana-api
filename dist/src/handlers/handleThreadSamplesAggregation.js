...
exports.handleThreadSamplesAggregation = handleThreadSamplesAggregation;
const utils_1 = require("../lib/utils");
async function handleThreadSamplesAggregation(params = {}) {
    try {
        const { beginTime = 'C-H1', endTime = 'C', statementHash = '%', passportAction = '%', aggregateBy = 'HASH', appName = '%', appUser = '%', workloadClass = '%' } = params || {};
        // 싱글 쿼트 이스케이프 함수
      ...
      ( SELECT
        ...
        ( SELECT
          ...
          ( SELECT                                                      /* Modification section */
              ...
              '${passportAction}' STATEMENT_ID,
              '%' STATEMENT_EXECUTION_ID,
              '%' DB_USER,
              '${escape(appName)}' APP_NAME,
              '${escape(appUser)}' APP_USER,
              '%' APP_SOURCE,
              '%' APP_COMP_NAME,
              '%' APP_COMP_TYPE,
              '%' LOCK_TYPE,
              ...
              '${escape(workloadClass)}' WORKLOAD_CLASS,
              '%' JOB_QUEUEING,
              -1  MIN_SAMPLES_TOTAL,
              1 TIME_SLICE_S,
              ' ' ONLY_DELTA_STORAGE_WAITS,
